...
//  Physics model per ship (all per-frame, Euler integration):
//    heave   (Y)        — buoyancy spring vs gravity
//    pitch   (X rot)    — bow/stern Archimedes moment
//    roll    (Z rot)    — port/starboard Archimedes moment
//    surge   (Z)        — forward drift
//    sway    (X)        — lateral drift from hits
//    yaw     (Y rot)    — slow weather-vane rotation
//
//  Impact events apply impulse vectors to the physics state.
// ─────────────────────────────────────────────────────────────────────────────
...
export default class ThreeWorld {
  constructor(canvas) {
    this.canvas   = canvas;
    this.ships    = {};     // key -> ShipPhysics object
    this._raf     = null;
  ...
  _buildLights() {
    ...
    this.scene.add(Object.assign(new THREE.PointLight(0xff2200,4,220), {position: new THREE.Vector3(0,3,-70)}));

    this._flashLight = new THREE.PointLight(0xff8822, 0, 140);
    this._flashLight.position.set(0,10,0);
    this.scene.add(this._flashLight);

    // Lantern lights on each ship position (filled in after ships built)
    this._lanternLights = [];
  }
  ...
  _buildOcean() {
    ...
    // Precompute wave function used for ship buoyancy
    this._waveTime = 0;
  }
  ...
  // Wave height at world (x,z) at time t — matches visual water
  _waveH(x, z, t) {
  ...
  _buildRain() {
    ...
    for(let i=0;i<N;i++) {
      pos[i*3]=(Math.random()-0.5)*220; pos[i*3+1]=Math.random()*90; pos[i*3+2]=(Math.random()-0.5)*220;
      vel[i]=0.45+Math.random()*0.55;
    }
    const geo=new THREE.BufferGeometry();
    geo.setAttribute('position',new THREE.BufferAttribute(pos,3));
    this._rainPos=pos; this._rainVel=vel; this._rainGeo=geo;
    this.rain=new THREE.Points(geo,new THREE.PointsMaterial({color:0x8899bb,size:0.13,transparent:true,opacity:0.32,sizeAttenuation:true}));
  ...
  _updateRain(dt) {
    ...
    for(let i=0;i<N;i++) {
      ...
      if(this._rainPos[i*3+1]<-1) {
        this._rainPos[i*3+1]=88;
        // tiny splash EventBus emit throttled
        if(Math.random()<0.004) EventBus.emit('rainSplash',{x:this._rainPos[i*3]});
      }
    }
  ...
  _createShip(isEnemy, worldX, worldRotY) {
    ...
    // ── Hull (lathe from profile curve) ──────────────────────────────────
    const profile = [
      new THREE.Vector2(0.0,  -3.8),   // keel
      new THREE.Vector2(1.2,  -3.5),
      new THREE.Vector2(2.8,  -2.8),
      new THREE.Vector2(4.2,  -1.2),
      new THREE.Vector2(4.6,   0.2),
      new THREE.Vector2(4.4,   1.5),   // waterline
      new THREE.Vector2(4.2,   2.8),
      new THREE.Vector2(3.8,   3.5),   // sheer
    ];
    const hullGeo = new THREE.LatheGeometry(profile, 24);
    // Squash Z to make hull elliptical (wider beam than depth)
    ...
    // ── Deck ─────────────────────────────────────────────────────────────
    const deckGeo = new THREE.BoxGeometry(9.2, 0.32, 4.0);
    ...
    // Plank lines
    for(let i=-4;i<=4;i++) {
      const pg=new THREE.BoxGeometry(9.0,0.04,0.06);
      const pm=new THREE.Mesh(pg,mat(isEnemy?0x2a1008:0x2a1408,1,0));
      pm.position.set(0,3.87,i*0.44);
      root.add(pm);
    }

    // ── Stern castle ─────────────────────────────────────────────────────
    const stern=new THREE.Mesh(new THREE.BoxGeometry(2.6,2.0,4.2),MAT[`${p}Deck`]);
    stern.position.set(4.0,4.8,0); stern.castShadow=true;
    root.add(stern);
    // Stern windows
    ...
    bow.position.set(-5.8,3.5,0); bow.castShadow=true;
    root.add(bow);
    ...
    bs.position.set(-7.2,5.5,0); bs.castShadow=true;
    root.add(bs);

    // ── Cannons (4 per broadside) ─────────────────────────────────────────
    this._buildCannons(root, p, isEnemy);

    // ── Main mast + rigging ───────────────────────────────────────────────
    this._buildMastSystem(root, p, isEnemy);

    // ── Figurehead ───────────────────────────────────────────────────────
    const fhGeo=new THREE.SphereGeometry(0.32,8,8);
    const fh=new THREE.Mesh(fhGeo,MAT.gold);
    fh.position.set(-6.8,4.2,0);
    root.add(fh);

    // ── Anchor chain ─────────────────────────────────────────────────────
    for(let i=0;i<5;i++) {
      const cg=new THREE.TorusGeometry(0.15,0.04,4,8);
      const cm=new THREE.Mesh(cg,MAT[`${p}Metal`]);
      cm.position.set(-4.2,-2.0+i*0.4,1.8);
      cm.rotation.y=i*0.6;
      root.add(cm);
    }

    // ── Wake foam ─────────────────────────────────────────────────────────
    const wakeGeo=new THREE.PlaneGeometry(2,12,1,8);
    const wake=new THREE.Mesh(wakeGeo, new THREE.MeshStandardMaterial({color:0x88aacc,transparent:true,opacity:0.18,roughness:1}));
    ...
    const phys = {
      ...
      // 6-DOF state
      y:0, vy:0,          // heave
      pitch:0, vpitch:0,  // pitch (X)
      roll:0,  vroll:0,   // roll  (Z)
      yaw:0,   vyaw:0,    // yaw   (Y on top of worldRotY)
      surgeZ:0,vsurgeZ:0, // surge (Z drift)
      swayX:0, vswayX:0,  // sway  (X drift from hits)
      // damage state
      hpPct:1,
      crackMeshes: [],
      isEnemy,
  ...
  _buildCannons(root, p, isEnemy) {
    const positions = [-2.8, -0.9, 0.9, 2.8];
    positions.forEach(bz => {
      // Port broadside
      ...
      cannon.position.set(-2.2,3.4,bz);
  ...
  _buildMastSystem(root, p, isEnemy) {
    const mastDefs=[
      {x:-1.0, height:16, yards:[{y:7,hw:4.2},{y:11,hw:3.2},{y:14.5,hw:2.2}]},  // foremast
      {x: 1.4, height:19, yards:[{y:8,hw:5.0},{y:12,hw:3.8},{y:16,hw:2.6}]},  // mainmast
      {x: 4.2, height:11, yards:[{y:5,hw:2.8},{y:8,hw:2.0}]},                 // mizzenmast
    ];

    mastDefs.forEach(md=>{
      // Mast pole
      const mg=new THREE.CylinderGeometry(0.12,0.19,md.height,10);
      const mast=new THREE.Mesh(mg,MAT[`${p}Deck`]);
      mast.position.set(md.x, 3.56+md.height/2, 0);
      mast.castShadow=true;
      root.add(mast);

      // Crow's nest on mainmast
      if(md.height===19){
        const nestGeo=new THREE.CylinderGeometry(0.9,0.7,0.6,12,1,true);
        const nest=new THREE.Mesh(nestGeo,MAT[`${p}Deck`]);
        nest.position.set(md.x,3.56+md.height-3.5,0);
        root.add(nest);
        // Lookout figure
      ...
      md.yards.forEach((yd,yi)=>{
        ...
        yard.position.set(md.x,3.56+yd.y,0);
        root.add(yard);

        // Sail (custom quad with billow)
        const nextY=md.yards[yi+1]?.y ?? yd.y+3.8;
        const sailH=nextY-yd.y-0.4;
        const sail=this._buildSail(yd.hw,sailH,MAT[`${p}Sail`],isEnemy);
        sail.position.set(md.x,3.56+yd.y+0.2,0);
        root.add(sail);
      ...
      // Forestay from top of mast to bowsprit tip
      this._addRope(root,MAT[`${p}Rope`],
    ...
    const flagMat=new THREE.MeshStandardMaterial({color:isEnemy?0xcc2222:0x009944,side:THREE.DoubleSide,roughness:0.9});
    const flag=new THREE.Mesh(flagGeo,flagMat);
    flag.position.set(1.4,3.56+19.4,0.9);
    root.add(flag);
    this._flag=flag;
  }

  _buildSail(halfWidth, height, material, isEnemy) {
    // Billowed sail via custom BufferGeometry
    const W=8, H=6;
    ...
    for(let j=0;j<=H;j++) for(let i=0;i<=W;i++) {
      ...
      const y=v*height;
      // Billow: parabolic bow outward
      const bulge=(1-(u*2-1)**2)*(1-(v*2-1)**2)*1.8;
      const z=bulge*(isEnemy?-1:1);
      verts.push(x,y,z);
      uvs.push(u,v);
    }
    for(let j=0;j<H;j++) for(let i=0;i<W;i++) {
  ...
  _crewFigure(x,y,z,p) {
    ...
    const body=new THREE.Mesh(new THREE.CapsuleGeometry(0.18,0.55,4,8),MAT[`${p}Deck`]);
    body.position.y=0.42;
    const head=new THREE.Mesh(new THREE.SphereGeometry(0.19,8,8),mat(0xddccaa,0.9,0));
    head.position.y=1.05;
    const hat=new THREE.Mesh(new THREE.CylinderGeometry(0.08,0.22,0.3,8),mat(0x111111,1,0));
    hat.position.y=1.32;
    const brim=new THREE.Mesh(new THREE.CylinderGeometry(0.28,0.28,0.06,8),mat(0x111111,1,0));
    brim.position.y=1.20;
  ...
  _updatePhysics(phys, dt) {
    ...
    // Sample wave height at 5 points on hull
    const hC  = this._waveH(bx,    0,  t);      // centre
    const hB  = this._waveH(bx-4.5,0,  t);      // bow
    const hSt = this._waveH(bx+4.5,0,  t);      // stern
    const hP  = this._waveH(bx,   -2,  t);      // port
    const hSb = this._waveH(bx,    2,  t);      // starboard

    // ── Heave ──
    const buyForce = (hC - phys.y) * 9.0;       // buoyancy spring
    const dragY    = -phys.vy * 2.8;             // water damping
    phys.vy += (buyForce + dragY) * dt;
    phys.y  += phys.vy * dt;

    // ── Pitch (bow/stern asymmetry) ──
    const pitchMoment = (hSt - hB) * 3.2;
    phys.vpitch += (pitchMoment - phys.pitch*4.5 - phys.vpitch*3.8) * dt;
    phys.pitch  += phys.vpitch * dt;

    // ── Roll (port/starboard) ──
    const rollMoment = (hSb - hP) * 2.8;
    phys.vroll += (rollMoment - phys.roll*5.0 - phys.vroll*4.2) * dt;
    phys.roll  += phys.vroll * dt;

    // ── Yaw (slow weather-vane drift) ──
    const yawDrift = Math.sin(t*0.14 + (phys.isEnemy?1.8:0)) * 0.004;
    phys.vyaw  += (yawDrift - phys.yaw*0.8 - phys.vyaw*1.6) * dt;
    phys.yaw   += phys.vyaw * dt;
    phys.yaw    = THREE.MathUtils.clamp(phys.yaw,-0.12,0.12);

    // ── Sway (lateral, from hits — spring-damper) ──
    phys.vswayX += (-phys.swayX*6.0 - phys.vswayX*4.5) * dt;
    phys.swayX  += phys.vswayX * dt;

    // ── Surge (forward drift — slow sinusoidal) ──
    phys.vsurgeZ += (-phys.surgeZ*1.2 - phys.vsurgeZ*1.0) * dt;
    phys.surgeZ  += phys.vsurgeZ * dt;
  ...
  applyImpact(key, opts={}) {
    const p = this.ships[key];
    if (!p) return;
    const { sway=0, heave=0, pitch=0, roll=0 } = opts;
    p.vswayX  += sway;
    p.vy      += heave;
    p.vpitch  += pitch;
    p.vroll   += roll;
  ...
  flashCannonLight(x, z, color=0xff8822) {
    ...
    const decay=()=>{
      ...
      this._flashLight.intensity*=0.76;
      if(this._flashLight.intensity>0.15) requestAnimationFrame(decay);
  ...
  shakeCamera(intensity=0.4, duration=400) {
    ...
    const shake=()=>{
      ...
      if(el>duration){ this.camera.position.copy(origin); return; }
      const f=(1-el/duration);
      this.camera.position.set(
        origin.x+(Math.random()-0.5)*intensity*f,
        origin.y+(Math.random()-0.5)*intensity*0.4*f,
        origin.z+(Math.random()-0.5)*intensity*0.3*f,
  ...
  _bindEvents() {
    ...
    // Physics impulses from PixiStage
    EventBus.on('impactImpulse', ({who,sway,heave,pitch,roll})=>{
      this.applyImpact(who,{sway,heave,pitch,roll});
    });
    // Cannon recoil: player ship pitches back when firing
    EventBus.on('cannonRecoil', ({who})=>{
      const sign = (who==='player') ? 1 : -1;
      this.applyImpact(who,{pitch: sign*0.018, heave: 0.12});
    });
  }
  ...
  // ── GLTF stub (no-op — we build ships procedurally now) ─────────────────
  loadShip() {} // left for API compatibility
