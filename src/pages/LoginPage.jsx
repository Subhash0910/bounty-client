...
function BgCanvas() {
  ...
  useEffect(() => {
    ...
    // shooting star
    let shootX = -1, shootY = -1, shootActive = false;
    const triggerShoot = () => {
      if (shootActive) return;
      shootX = Math.random() * W * 0.6;
      shootY = Math.random() * H * 0.3;
      shootActive = true;
      setTimeout(() => { shootActive = false; }, 700);
    ...
    const draw = () => {
      ...
      if (shootActive) {
        shootProgress += 0.045;
        const sx = shootX + shootProgress * 160;
        const sy = shootY + shootProgress * 50;
        const grad = ctx.createLinearGradient(sx - 80, sy - 25, sx, sy);
        grad.addColorStop(0, 'transparent'); grad.addColorStop(1, 'rgba(200,230,255,0.85)');
        ctx.strokeStyle = grad; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.moveTo(sx - 80, sy - 25); ctx.lineTo(sx, sy); ctx.stroke();
        if (shootProgress > 1) { shootActive = false; shootProgress = 0; }
      }
      // ocean waves
      wv += 0.35;
      for (let i = 0; i < 6; i++) {
...
export default function LoginPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      localStorage.setItem('bounty', res.data.bounty ?? 0);
      navigate('/world');
    } catch (err) {
        ...
        <div style={{
          ...
          <div style={{ padding: '2.5rem 2.6rem 2.1rem' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '2.2rem' }}>
              ...
              <h2 style={{ color: '#00f5d4', fontSize: '1.35rem', fontWeight: 900, letterSpacing: '0.28em', margin: 0, textShadow: '0 0 30px rgba(0,245,212,0.35)' }}>LOGIN</h2>
              <p style={{ color: '#6a9aaa', fontSize: '10px', letterSpacing: '4px', margin: '7px 0 0' }}>RETURN TO THE OCEAN</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              >{loading ? '▸ SAILING IN...' : 'ENTER'}</button>
            </form>
