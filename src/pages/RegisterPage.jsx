...
const S = {
  ...
  success: { color: '#00f5d4', fontSize: '0.85rem', marginBottom: '1rem', textAlign: 'center' },
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input style={S.input} type="text" name="handle" value={form.handle} onChange={handleChange} required maxLength={30} placeholder="Your pirate name" />
          <label style={S.label}>EMAIL</label>
          <input style={S.input} type="email" name="email" value={form.email} onChange={handleChange} required />
          <label style={S.label}>PASSWORD</label>
          <input style={S.input} type="password" name="password" value={form.password} onChange={handleChange} required minLength={6} />
          <button style={S.btn} type="submit" disabled={loading}>
            {loading ? 'REGISTERING...' : 'JOIN THE OCEAN'}
          </button>
        </form>
        <Link to="/login" style={S.link}>Already sailing? Login</Link>
      </div>
    </div>
  );
}
