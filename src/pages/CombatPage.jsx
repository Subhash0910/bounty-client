  ...
  return (
    <button
      ...
      style={{
        ...
        background: active
          ? `linear-gradient(160deg, ${color}1a 0%, ${color}0a 100%)`
          : 'linear-gradient(160deg, rgba(4,9,20,0.88) 0%, rgba(3,7,16,0.72) 100%)',
        border: `1px solid ${disabled ? '#111e2e' : active ? color : color + '55'}`,
        borderRadius: '2px',
        boxShadow: active
          ? `0 0 36px ${color}44, inset 0 1px 0 ${color}22`
          : disabled ? 'none' : `0 0 10px ${color}18, inset 0 1px 0 ${color}10`,
        color: disabled ? '#1a2a3a' : color,
    ...
    >
      ...
      <span style={{ fontWeight: 700, fontSize: '13px', letterSpacing: '4px' }}>{label}</span>
      <span style={{ fontSize: '10px', color: disabled ? '#162030' : '#445566', letterSpacing: '0.5px', textAlign: 'center', padding: '0 8px' }}>{sub}</span>
    </button>
  );
}
...
export default function CombatPage() {
  ...
  // ── Boot Phaser then fetch island + sail ─────────────────────────────────
  useEffect(() => {
  ...
  // Use island type + name as enemy label in HP bar (passed via Phaser event)
  const islandLabel = island?.name ? island.name.toUpperCase().slice(0, 14) : 'ENEMY';
  const poster   = JSON.parse(localStorage.getItem('wantedPoster') || 'null');

  // Pass enemy label to scene once island is loaded
  useEffect(() => {
    if (island && phaserRef.current) {
      phaserRef.current.events.emit('setEnemyLabel', islandLabel);
    }
  }, [island, islandLabel]);
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', overflow:'hidden', position:'relative', fontFamily:MONO, background:'#010408' }}>
      ...
      <div style={{
        ...
        background:'linear-gradient(180deg,rgba(1,4,8,0.97) 0%,rgba(1,4,8,0.76) 70%,transparent 100%)',
        display:'flex', alignItems:'center', padding:'0 28px', gap:'18px', zIndex:30,
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'1px', flex:1, overflow:'hidden', minWidth:0 }}>
          ...
          <span style={{ color:'#152535', fontSize:'11px', fontStyle:'italic', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
            {island?.lore || ''}
          </span>
        </div>
        <div style={{ textAlign:'center', flexShrink:0 }}>
          <div style={{ color:'#ffd700', fontSize:'11px', letterSpacing:'8px', fontWeight:700 }}>ROUND {round}</div>
          <div style={{ color:'#152535', fontSize:'9px', letterSpacing:'2px', marginTop:'2px' }}>COMBAT</div>
        </div>
        <button
          onClick={() => navigate('/world')}
          style={{
            background:'transparent', border:'1px solid rgba(255,68,102,0.18)',
            color:'#1e2a35', fontFamily:MONO, fontSize:'11px',
            padding:'5px 16px', cursor:'pointer', flexShrink:0,
            letterSpacing:'3px', transition:'all 0.18s', borderRadius:'1px',
          }}
          ...
          onMouseLeave={e=>{ e.currentTarget.style.borderColor='rgba(255,68,102,0.18)'; e.currentTarget.style.color='#1e2a35'; e.currentTarget.style.boxShadow='none'; }}
        >← BACK</button>
      </div>
      ...
      <div style={{
        ...
        {log.map((line, i) => (
          <div key={`${line}-${i}`} style={{
            background:'rgba(2,5,14,0.82)', backdropFilter:'blur(8px)',
            borderLeft:`2px solid ${i===0?'#00f5d466':'#00f5d415'}`,
            borderRight:'1px solid rgba(0,245,212,0.04)',
            padding:'7px 18px',
            color: i===0 ? '#88aabc' : '#1e3040',
            fontSize:'12px', letterSpacing:'0.3px', lineHeight:'1.5',
      ...
      <div style={{
        ...
        background:'linear-gradient(0deg,rgba(1,4,8,0.99) 0%,rgba(1,4,8,0.88) 55%,transparent 100%)',
        display:'flex', alignItems:'center', justifyContent:'center',
        gap:'22px', zIndex:30, paddingBottom:'18px',
        borderTop:'1px solid rgba(0,245,212,0.03)',
      }}>
        <ActionBtn icon="⚔️"  label="ATTACK"     color="#00f5d4" sub="15–25 DMG  •  10–20 TAKEN" disabled={btnOff}           onClick={() => takeTurn('ATTACK')} />
        <div style={{ width:'1px', height:'52px', background:'linear-gradient(180deg,transparent,#0d1f2e,transparent)' }} />
      ...
      {/* ══ ERROR ══════════════════════════════════════════════════════════════════════════ */}
      {error && (
        <div style={{ position:'absolute',inset:0,background:'rgba(1,4,8,0.94)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:50,gap:'1.5rem' }}>
      ...
      {overlay === 'victory' && (
        <div style={{
          ...
          background:'radial-gradient(ellipse at center,rgba(0,35,30,0.97) 0%,rgba(1,4,8,0.99) 70%)',
          display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          zIndex:50,gap:'14px',animation:'fadeOverlay 0.6s ease',
        }}>
          <div style={{ width:'140px',height:'1px',background:'linear-gradient(90deg,transparent,#ffd700,transparent)',marginBottom:'4px' }} />
          <p style={{ fontFamily:MONO,fontSize:'10px',color:'#233a1a',letterSpacing:'10px',margin:0 }}>ISLAND CLAIMED</p>
          ...
          <div style={{ width:'140px',height:'1px',background:'linear-gradient(90deg,transparent,#00f5d444,transparent)',margin:'4px 0' }} />
          {poster && <WantedPoster player={poster} />}
          <button
            style={{
              background:'linear-gradient(135deg,rgba(0,245,212,0.12),rgba(0,245,212,0.05))',
              ...
              boxShadow:'0 0 28px #00f5d433',transition:'all 0.18s',borderRadius:'1px',
            }}
            onMouseEnter={e=>{e.currentTarget.style.boxShadow='0 0 55px #00f5d466';e.currentTarget.style.background='rgba(0,245,212,0.16)';}}
            onMouseLeave={e=>{e.currentTarget.style.boxShadow='0 0 28px #00f5d433';e.currentTarget.style.background='linear-gradient(135deg,rgba(0,245,212,0.12),rgba(0,245,212,0.05))';}} 
            onClick={()=>navigate('/world')}
      ...
      {overlay === 'defeat' && (
        <div style={{
          position:'absolute',inset:0,
          background:'radial-gradient(ellipse at center,rgba(25,0,6,0.97) 0%,rgba(1,4,8,0.99) 70%)',
          display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
          zIndex:50,gap:'14px',animation:'fadeOverlay 0.6s ease',
        }}>
          <div style={{ width:'140px',height:'1px',background:'linear-gradient(90deg,transparent,#ff4466,transparent)',marginBottom:'4px' }} />
          ...
          <p style={{ fontFamily:MONO,fontSize:'18px',color:'#ff4466',margin:0 }}>
            {Math.abs(combat?.bountyChange||0).toLocaleString()} BOUNTY LOST
          </p>
          <p style={{ fontFamily:MONO,fontSize:'12px',color:'#1e2e3e',fontStyle:'italic',margin:0,letterSpacing:'1px' }}>
            The ocean does not forgive weakness.
          </p>
          <div style={{ width:'140px',height:'1px',background:'linear-gradient(90deg,transparent,#ff446644,transparent)',margin:'4px 0' }} />
