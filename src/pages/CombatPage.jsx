...
// Move cooldowns in rounds
const COOLDOWNS = { EVADE: 1, SABOTAGE: 2, BOARD: 0, ATTACK: 0, NEGOTIATE: 0 };

// RPS counter system: what each move beats
const COUNTERS = {
  EVADE:   'AGGRESSIVE',
  BOARD:   'DEFENSIVE',
  ATTACK:  'DESPERATE',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', width: '240px' }}>
      ...
      <div style={{
        ...
        outline: flash ? `1px solid ${barColor}` : 'none',
        boxShadow: flash ? `0 0 18px ${barColor}88` : 'none',
        ...
        {danger && (
          <div style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, background: 'rgba(255,68,102,0.15)', animation: 'hpPulse 0.8s ease-in-out infinite' }} />
        )}
      </div>
      {effects.length > 0 && (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', width: '130px' }}>
      ...
      <div style={{ width: '100%', height: '6px', background: 'rgba(0,0,0,0.7)', border: `1px solid ${danger ? '#ff880055' : '#1a3040'}`, position: 'relative', overflow: 'hidden' }}>
        <div style={{
          ...
          boxShadow: danger ? '0 0 10px #ff880099' : 'none',
...
function StanceBadge({ stance }) {
  ...
  return (
    <div title={m.tip} style={{
      ...
      padding: '5px 12px', border: `1px solid ${m.color}55`,
...
function RollBadge({ roll, label }) {
  ...
  return (
    <div style={{
      ...
      padding: '5px 10px', border: `1px solid ${high ? '#ffd70055' : low ? '#ff446655' : '#1a3a4a'}`,
      background: high ? 'rgba(255,215,0,0.08)' : low ? 'rgba(255,68,102,0.08)' : 'rgba(0,6,14,0.7)',
      minWidth: '50px',
    }}>
...
function LogLine({ text, index }) {
  ...
  return (
    <div style={{
      background: index === 0 ? 'rgba(0,10,24,0.97)' : 'rgba(0,6,14,0.75)',
      ...
      animation: index === 0 ? 'logSlideIn 0.22s ease' : 'none',
      borderBottom: '1px solid rgba(0,20,40,0.4)',
    }}>{text}</div>
  );
}

function ActionBtn({ icon, label, sub, color, disabled, onClick, hotkey, tag, cooldownLeft, isCounter }) {
    ...
    <button
      ...
      style={{
        ...
        background: onCd
          ? 'rgba(0,0,0,0.6)'
          : active
            ? `linear-gradient(160deg,${btnColor}28,${btnColor}0e)`
            : disabled ? 'rgba(2,6,14,0.5)' : 'rgba(4,10,24,0.92)',
        border: `1px solid ${onCd ? '#1a2a3a' : disabled ? 'rgba(255,255,255,0.05)' : active ? btnColor : btnColor + '55'}`,
        overflow: 'hidden',
        boxShadow: isCounter && !disabled && !onCd
          ? `0 0 28px #ffd70066, inset 0 0 20px #ffd70011`
          : active ? `0 0 36px ${btnColor}55` : `0 0 8px ${btnColor}14`,
        color: onCd ? '#2a3a4a' : disabled ? 'rgba(255,255,255,0.1)' : btnColor,
        fontFamily: MONO, cursor: (disabled || onCd) ? 'not-allowed' : 'pointer',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        gap: '3px', padding: '0',
        transition: 'all 0.12s ease',
        transform: active ? 'translateY(-3px)' : 'none',
        opacity: (disabled && !onCd) ? 0.22 : onCd ? 0.45 : 1,
        userSelect: 'none',
      }}
    >
      {/* Top glow line */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: active ? `linear-gradient(90deg,transparent,${btnColor}cc,transparent)` : 'transparent', transition: 'background 0.12s' }} />
      {/* Hotkey */}
      <div style={{ position: 'absolute', top: '5px', right: '7px', color: onCd ? '#1a2a3a' : `${btnColor}77`, fontSize: '8px', letterSpacing: '1px' }}>[{hotkey}]</div>
      {/* Tag or cooldown */}
      {onCd
        ? <div style={{ position: 'absolute', top: '5px', left: '7px', fontSize: '8px', color: '#ff880099', letterSpacing: '2px', background: 'rgba(255,136,0,0.1)', padding: '1px 5px' }}>CD {cooldownLeft}</div>
        : tag && <div style={{ position: 'absolute', top: '5px', left: '7px', fontSize: '7px', color: isCounter ? '#ffd70099' : `${btnColor}99`, letterSpacing: '2px', background: `${btnColor}18`, padding: '1px 5px' }}>{tag}</div>
      }
      {/* Counter glow ring */}
      {isCounter && !disabled && !onCd && (
        <div style={{ position: 'absolute', inset: '2px', border: '1px solid #ffd70033', pointerEvents: 'none', animation: 'counterPulse 1.2s ease-in-out infinite' }} />
      )}
      <span style={{ fontSize: '22px', lineHeight: 1, filter: (disabled || onCd) ? 'grayscale(1) opacity(0.3)' : isCounter ? 'drop-shadow(0 0 8px #ffd700)' : 'drop-shadow(0 0 5px currentColor)' }}>{icon}</span>
      <span style={{ fontWeight: 900, fontSize: '11px', letterSpacing: '4px', marginTop: '2px' }}>{label}</span>
      <span style={{ fontSize: '8px', color: onCd ? '#1a2a3a' : disabled ? 'rgba(255,255,255,0.06)' : '#5a7a8a', letterSpacing: '0.5px', textAlign: 'center', padding: '0 8px', lineHeight: 1.4 }}>{sub}</span>
    </button>
  );
}
...
export default function CombatPage() {
  ...
  const [pFlash,   setPFlash]   = useState(false);  // player HP flash
  const [eFlash,   setEFlash]   = useState(false);  // enemy HP flash

  // Move cooldowns: { EVADE: 0, SABOTAGE: 0, ... }
  ...
  // ── Tick down cooldowns each round ─────────────────────────────────────
  ...
  }, [combat, bounty, cooldowns, tickCooldowns]);

  // ── Bounty spend: Repair Hull (+30 HP) ─────────────────────────────────
  const repairHull = useCallback(async () => {
    ...
    setCombat(prev => ({ ...prev, playerHealth: Math.min(100, prev.playerHealth + 30) }));
  ...
  // Which move counters current stance
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        display: 'flex', alignItems: 'center', padding: '0 20px', gap: '16px', zIndex: 30,
        borderBottom: `2px solid ${typeColor}33`,
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ color: '#ffffff', fontWeight: 900, fontSize: '14px', letterSpacing: '2px', whiteSpace: 'nowrap' }}>
              {island?.name || '——'}
            </span>
          ...
          <div style={{ color: '#6a8a9a', fontSize: '10px', fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: '2px', paddingLeft: '14px' }}>
            {island?.lore || ''}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
          ...
          {canRepair && (
            <button
              onClick={repairHull}
              style={{
                background: 'rgba(0,245,212,0.08)', border: '1px solid #00f5d455',
                color: '#00f5d4', fontFamily: MONO, fontSize: '9px', padding: '5px 12px',
                cursor: 'pointer', letterSpacing: '2px', transition: 'all 0.14s',
                flexShrink: 0,
              }}
      ...
      {combat && (
        <div style={{
          ...
          <HealthBar
            ...
            color={typeColor} side="right" effects={enemyEffects} flash={eFlash}
          />
        </div>
      )}
      ...
      <div style={{
        ...
        pointerEvents: 'none',
        border: '1px solid rgba(0,20,40,0.7)',
      ...
      <div style={{
        ...
        gap: '8px', zIndex: 30, paddingBottom: '10px',
        borderTop: `2px solid ${typeColor}22`,
        ...
        <ActionBtn
          ...
          cooldownLeft={cooldowns.ATTACK}
          isCounter={counterMove === 'ATTACK'}
          tag={counterMove === 'ATTACK' ? '⚡ COUNTER' : undefined}
        />
        ...
        <ActionBtn
          ...
          tag={isAnchored ? 'LOCKED' : counterMove === 'BOARD' ? '⚡ COUNTER' : 'RISKY'}
          disabled={btnOff || isAnchored}
          onClick={() => takeTurn('BOARD')}
          cooldownLeft={cooldowns.BOARD}
          isCounter={!isAnchored && counterMove === 'BOARD'}
        />
        <div style={{ width: '1px', height: '55px', background: 'rgba(0,245,212,0.08)' }} />
        <ActionBtn
          icon="💨" label="EVADE" hotkey="3" color="#4a9eff"
          sub="Dodge next hit · may ANCHOR"
          tag={isAnchored ? 'LOCKED' : counterMove === 'EVADE' ? '⚡ COUNTER' : undefined}
          disabled={btnOff || isAnchored}
          onClick={() => takeTurn('EVADE')}
          cooldownLeft={cooldowns.EVADE}
          isCounter={!isAnchored && counterMove === 'EVADE'}
        />
        <div style={{ width: '1px', height: '55px', background: 'rgba(0,245,212,0.08)' }} />
        <ActionBtn
          ...
          cooldownLeft={cooldowns.SABOTAGE}
          isCounter={false}
        />
        <div style={{ width: '1px', height: '55px', background: 'rgba(0,245,212,0.08)' }} />
        <ActionBtn
          ...
          cooldownLeft={cooldowns.NEGOTIATE}
          isCounter={false}
        />
      </div>
      ...
      {error && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(1,3,10,0.98)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 50, gap: '1.5rem' }}>
          ...
          <button
            style={{ background: 'transparent', border: '1px solid #00f5d4', color: '#00f5d4', padding: '0.7rem 2rem', fontFamily: MONO, cursor: 'pointer', letterSpacing: '3px', fontSize: '11px', transition: 'all 0.14s' }}
      ...
      {overlay === 'victory' && (
        <div style={{
          ...
          <button
            style={{ background: 'linear-gradient(135deg,rgba(0,245,212,0.15),rgba(0,245,212,0.06))', border: '1px solid #00f5d4', color: '#00f5d4', padding: '10px 40px', fontFamily: MONO, fontSize: '12px', cursor: 'pointer', letterSpacing: '5px', marginTop: '8px', boxShadow: '0 0 30px #00f5d444', transition: 'all 0.14s' }}
      ...
      {overlay === 'defeat' && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '14px', marginTop: '8px' }}>
            <button
              style={{ background: 'rgba(255,68,102,0.10)', border: '1px solid #ff446699', color: '#ff4466', padding: '10px 28px', fontFamily: MONO, fontSize: '11px', cursor: 'pointer', letterSpacing: '3px', transition: 'all 0.14s' }}
            ...
            >🔄 TRY AGAIN</button>
            <button
              style={{ background: 'rgba(0,245,212,0.06)', border: '1px solid #00f5d444', color: '#00f5d4', padding: '10px 28px', fontFamily: MONO, fontSize: '11px', cursor: 'pointer', letterSpacing: '3px', transition: 'all 0.14s' }}
      ...
      <style>{`
        ...
        @keyframes counterPulse { 0%,100%{opacity:0.6;box-shadow:0 0 0 0 rgba(255,215,0,0.4);}50%{opacity:1;box-shadow:0 0 0 4px rgba(255,215,0,0);} }
        @keyframes screenShake  {
          0%{transform:none} 15%{transform:translate(-6px,3px) rotate(-0.4deg)}
          30%{transform:translate(6px,-3px) rotate(0.3deg)} 50%{transform:translate(-4px,2px)}
          70%{transform:translate(4px,-2px)} 85%{transform:translate(-2px,1px)} 100%{transform:none}
        }
