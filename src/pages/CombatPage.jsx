...
export default function CombatPage() {
  ...
  // PixiJS canvas (transparent overlay)
  ...
  useEffect(() => {
    ...
    // Three.js — the 3D ocean world
    const world = new ThreeWorld(threeCanvasRef.current);
    threeWorldRef.current = world;

    // Try loading 3D ships — falls back silently if GLTFs not present
    ...
    (async () => {
      try {
        ...
        // Telegraph enemy stance on load
    ...
    try {
      ...
      if (pixi) {
        if (approach === 'ATTACK') {
          ...
          const success = dmgTaken === 0;
          pixi.playEvade(success);
          AudioManager.play('whoosh');
        } else if (approach === 'SABOTAGE') {
          pixi.playSabotage(true);
          AudioManager.play('sabotage');
        } else if (approach === 'NEGOTIATE') {
          const success = state.status==='PLAYER_WON' || state.enemyHealth < prevEHP;
          pixi.playNegotiate(success);
          AudioManager.play(success ? 'coins' : 'whoosh');
        }
      }
      ...
      if (state.status==='PLAYER_WON') {
        ...
        setTimeout(()=>{
          setOverlay('victory');
          if (overlayRef.current) gsap.from(overlayRef.current,{opacity:0,scale:0.96,duration:0.5,ease:'power2.out'});
        }, 1200);
      }
      if (state.status==='PLAYER_LOST') {
        ...
        setTimeout(()=>{
          setOverlay('defeat');
          if (overlayRef.current) gsap.from(overlayRef.current,{opacity:0,scale:1.04,duration:0.5,ease:'power2.out'});
        }, 1400);
      }
    } catch {
  ...
  return (
    <div style={{ width:'100vw', height:'100vh', overflow:'hidden', position:'relative', fontFamily:MONO, background:'#010408' }}>
      ...
      {/* ══ THREE.JS CANVAS — the 3D ocean world (bottom) */}
      <canvas
        ref={threeCanvasRef}
        style={{ position:'absolute', inset:0, width:'100%', height:'100%', zIndex:0 }}
      />

      {/* ══ PIXI CANVAS — ships, particles, FX (transparent overlay) */}
      <canvas
        ref={pixiCanvasRef}
      ...
      {/* ══ ACTION BAR — WeaponCards replace ActionBtn */}
